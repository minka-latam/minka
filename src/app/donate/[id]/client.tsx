...
import { ArrowRight, QrCode, CreditCard, ArrowUp } from "lucide-react";
...
type CardOrigin = "bolivian" | "foreign";
...
const PAYMENT_METHODS = [
  ...
  },
  {
    ...
    description:
      "Elige tarjeta boliviana (BOB) o extranjera (USD) antes de continuar.",
...
export function DonatePageContent({ campaignId }: { campaignId: string }) {
  ...
  const [cardOrigin, setCardOrigin] = useState<CardOrigin>("bolivian");
  const [minkaContribution, setMinkaContribution] = useState<number>(0);
  ...
  const [wantsSignInAfterDonation, setWantsSignInAfterDonation] =
    useState(false);
  const [wantsAnonymousDonation, setWantsAnonymousDonation] = useState(false);
  ...
  // Read redirect parameters from the hosted card checkout.
  ...
  useEffect(() => {
    ...
    return () => {
      window.removeEventListener("pageshow", resetCardRedirectState);
      ...
      try {
        ...
        if (stored) {
          ...
          if (
            ...
            setQrAccessToken(pendingDonation.qrAccessToken ?? null);
            setSelectedAmount(pendingDonation.amount);
            setPaymentMethod(pendingDonation.paymentMethod);
            const pendingTipAmount = Number(pendingDonation.tipAmount ?? 0);
  ...
  // Hosted card checkout redirect + status polling.
  useEffect(() => {
    ...
    // The poller intentionally starts only when the checkout redirect ID changes.
  ...
  const cardCurrency = cardOrigin === "foreign" ? "USD" : "BOB";
  const selectedCurrency = paymentMethod === "card" ? cardCurrency : "BOB";
  const currencyPrefix = selectedCurrency === "USD" ? "$" : "Bs.";
  const donationAmounts =
    selectedCurrency === "USD" ? DONATION_AMOUNTS_USD : DONATION_AMOUNTS_BS;
  const isDonationAmountValid =
    Number.isFinite(donationAmount) &&
    donationAmount >= 1 &&
    donationAmount <= 50000;
  const isPaymentFormReady = Boolean(paymentMethod) && isDonationAmountValid;
  const isDonationAnonymous = user ? wantsAnonymousDonation : true;

  const saveDonationClaimIntent = (intent: DonationClaimIntent | null) => {
    if (!intent || !intent.donationId || !intent.claimToken) return null;

    localStorage.setItem(DONATION_CLAIM_INTENT_KEY, JSON.stringify(intent));
  ...
  const getStoredDonationClaimIntent = (
    ...
    if (
      ...
      return {
        ...
        campaignId,
        createdAt: pendingCardCheckout.createdAt || new Date().toISOString(),
      };
    }
    ...
    if (
      ...
      return {
        ...
        campaignId,
        createdAt: pendingDonation.createdAt || new Date().toISOString(),
      };
    }
  ...
  const shouldRedirectToSignupAfterDonation = (completedDonationId: string) => {
    try {
      const storedCheckout = sessionStorage.getItem(PENDING_CARD_CHECKOUT_KEY);
      if (storedCheckout) {
        const pendingCheckout = JSON.parse(storedCheckout);
        if (pendingCheckout?.donationId === completedDonationId) {
          return Boolean(pendingCheckout.wantsAccountAfterDonation);
        }
      }

      const storedDonation = localStorage.getItem(PENDING_DONATION_KEY);
      if (storedDonation) {
        const pendingDonation = JSON.parse(storedDonation);
        if (pendingDonation?.donationId === completedDonationId) {
          return Boolean(pendingDonation.wantsAccountAfterDonation);
        }
      }
    } catch {
  ...
  const shouldRedirectToSignInAfterDonation = (completedDonationId: string) => {
    try {
      const storedCheckout = sessionStorage.getItem(PENDING_CARD_CHECKOUT_KEY);
      if (storedCheckout) {
        const pendingCheckout = JSON.parse(storedCheckout);
        if (pendingCheckout?.donationId === completedDonationId) {
          return Boolean(pendingCheckout.wantsSignInAfterDonation);
        }
      }
      ...
      if (storedDonation) {
        const pendingDonation = JSON.parse(storedDonation);
        if (pendingDonation?.donationId === completedDonationId) {
          return Boolean(pendingDonation.wantsSignInAfterDonation);
        }
      }
    } catch {
    ...
    try {
      ...
      for (let i = 0; i < maxAttempts; i++) {
        ...
        if (res.ok && data?.success) {
          ...
          const dbCurrency =
            data?.donation?.providerCurrency === "USD" ? "USD" : "BOB";
          setCardOrigin(dbCurrency === "USD" ? "foreign" : "bolivian");

          // 1) amount: set either selectedAmount (if matches predefined) or customAmount
          const dbDonationAmounts =
            dbCurrency === "USD" ? DONATION_AMOUNTS_USD : DONATION_AMOUNTS_BS;
          const matchesPreset = dbDonationAmounts.some(
            (o) => o.value === dbAmount,
          );
          if (matchesPreset) {
    ...
    if (/^\d*(\.\d{0,2})?$/.test(value)) {
      ...
      if (!value || numValue <= 50000) {
        ...
        toast({
          ...
          description: `El monto máximo de donación es ${currencyPrefix} 50,000.`,
          variant: "destructive",
        });
      }
    }
  ...
  const handleCardOriginSelect = (origin: CardOrigin) => {
    if (origin === cardOrigin) return;
  ...
  const handleConfirmDonation = async () => {
    ...
    try {
      if (selectedMethod === "card") {
        const response = await fetch("/api/transoft/payment", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          credentials: "include",
          body: JSON.stringify({
            campaignId,
            clientAuthState: user?.id ? "authenticated" : "anonymous",
            ...
            customAmount: !selectedAmount && Boolean(customAmount),
          }),
        });
        const data = await response.json().catch(() => null);
        if (!response.ok || !data?.url || !data?.donationId) {
          const messages: Record<string, string> = {
            PAYMENT_PROVIDER_NOT_CONFIGURED:
              "El pago con tarjeta todavía no está configurado.",
            USD_ACCOUNTING_RATE_NOT_CONFIGURED:
              "Los pagos en USD todavía no tienen configurado el tipo de cambio contable de Minka.",
            UNSUPPORTED_CARD_CURRENCY:
              "La moneda seleccionada no está disponible para pagos con tarjeta.",
            PAYMENT_SESSION_REJECTED:
              "Transoft rechazó la sesión de pago. Revisa el monto e inténtalo nuevamente.",
            PAYMENT_PROVIDER_UNAVAILABLE:
              "Transoft no está disponible en este momento. Inténtalo nuevamente en unos minutos.",
          };
          throw new Error(
            messages[data?.error] || "No pudimos iniciar el pago con tarjeta.",
          );
        }
        ...
        setInfoMessage("Abriendo el checkout seguro de Transoft...");
  ...
  const campaignTitle =
    campaign?.title || "Nombre de la campaña sin especificar";

  const campaignImage =
    campaign?.media?.find((m) => m.is_primary)?.preview_url ||
    campaign?.media?.find((m) => m.is_primary)?.media_url ||
                  ...
                  <div className="border-t border-gray-200 pt-8">
                      ...
                      <div className="mb-7">
                        <fieldset>
                          <legend className="mb-3 text-base font-semibold text-gray-900">
                            ¿Qué tipo de tarjeta usarás?
                          </legend>
                          <div
                            className="grid grid-cols-1 gap-3 sm:grid-cols-2"
                            role="radiogroup"
                            aria-label="Tipo de tarjeta y moneda del cobro"
                          >
                            {(
                              [
                                {
                                  id: "bolivian",
                                  label: "Tarjeta boliviana",
                                  detail: "Cobro en BOB · Bs.",
                                },
                                {
                                  id: "foreign",
                                  label: "Tarjeta extranjera",
                                  detail: "Cobro en USD · $",
                                },
                              ] as const
                            ).map((option) => {
                              const isSelected = cardOrigin === option.id;
                              return (
                                <button
                                  key={option.id}
                                  type="button"
                                  ...
                                  className={`flex items-center gap-3 rounded-lg border p-4 text-left transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#2c6e49] focus-visible:ring-offset-2 ${
                                    isSelected
                                      ? "border-[#2c6e49] bg-[#f5f7e9]"
                                      : "border-gray-300 bg-white hover:border-[#2c6e49]"
                                  }`}
                                  onClick={() =>
                                    handleCardOriginSelect(option.id)
                                  }
                                >
                                  <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full border border-gray-400 bg-white">
                                    {isSelected && (
                                      <span className="h-2.5 w-2.5 rounded-full bg-[#2c6e49]" />
                                    )}
                                  </span>
                                  <span>
                        ...
                        <p className="mt-3 text-sm leading-relaxed text-gray-600">
                          Esta selección define la moneda que enviaremos a
                          Transoft y que verás en el checkout.
                        </p>
                      </div>
                    )}
                    <h3 className="text-lg font-semibold text-black mb-4">
                    ...
                    <div className="mt-16 mb-8">
                      <p className="text-sm text-black mb-3">
                        ¿Quieres también apoyar a Minka? La contribución
                        voluntaria es opcional y ayuda a sostener la plataforma.
                      </p>
                    ...
                    <div className="border-t border-gray-200 pt-5 mb-6">
                      ...
                      <div className="space-y-2 text-sm">
                        ...
                        <div className="flex justify-between gap-4">
                          <span className="text-gray-600">Tip a Minka</span>
                          <span className="font-medium">
                        ...
                        <div className="mb-6 rounded-lg border border-[#2c6e49]/25 bg-[#f5f7e9] p-4">
                          ...
                          >
                            ...
                            <span className="text-sm text-gray-800">
                              ...
                              podrás ver tu historial, guardar favoritas y
                      ...
                      <div className="mb-6 rounded-lg border border-gray-200 bg-white p-4">
                        ...
                        >
                          <Checkbox
                            ...
                            onCheckedChange={(checked) =>
                              setWantsAnonymousDonation(checked === true)
                            }
                            className="mt-1"
                          />
                          <span className="text-sm text-gray-800">
                  ...
                  {paymentMethod && reviewState ? (
                    <div className="flex flex-col sm:flex-row gap-3">
                      <Button
                        ...
                        onClick={() => {
                          const id =
                            activeDonationIdRef.current || activeDonationId;
            ...
            <div className="bg-[#FFFDF7] text-center p-8">
              ...
              {!user && (
                <p className="mt-3 text-sm text-[#2c6e49] font-medium">
                  Crea una cuenta para dejar tu nombre en la donación que
                  hiciste y seguir el impacto
                </p>
              )}
              ...
              {!user && (
                <div className="mt-5 rounded-lg bg-[#f5f7e9] p-4 text-left">
                  ...
                  <p className="mt-1 text-sm text-gray-700">
                    Crear una cuenta permite dejar tu nombre si lo deseas en las
                    donaciones, puedes también dejar mensajes de apoyo a la
                    causa, guardar tus campañas favoritas, revisar tu historial
                    de donaciones, volver fácilmente para ver actualizaciones
