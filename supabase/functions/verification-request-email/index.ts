...
type VerificationRequestEmailPayload = {
  campaignId?: string;
...
Deno.serve(async (request) => {
  ...
  const payload = (await request.json().catch(() => null)) as
    | VerificationRequestEmailPayload
    | null;
  ...
  const docsCount = Number(payload.supportingDocsCount || 0);
  ...
  const text = [
    ...
    `Documentos de respaldo: ${docsCount}`,
    "",
    `Ver campaña: ${payload.campaignUrl}`,
    `Panel de verificaciones: ${payload.dashboardUrl}`,
    "",
            ...
            <tr>
              <td style="padding:28px;">
                ...
                <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="border-collapse:collapse;margin:0 0 24px;">
                  <tr>
                    <td style="padding:12px 0;border-bottom:1px solid #eef0ea;color:#6a7468;font-size:13px;width:150px;">Campaña</td>
                  ...
                  <tr>
                    <td style="padding:12px 0;border-bottom:1px solid #eef0ea;color:#6a7468;font-size:13px;">Documentos</td>
                    <td style="padding:12px 0;border-bottom:1px solid #eef0ea;color:#172017;font-size:14px;">${docsCount}</td>
                  </tr>
                  <tr>
                ...
                <p style="margin:0;color:#6a7468;font-size:12px;line-height:1.5;">
                  Este aviso se envió automáticamente porque una campaña solicitó verificación.
                </p>
              </td>
            </tr>
          </table>
